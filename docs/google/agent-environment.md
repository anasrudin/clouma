...
For **private Git repositories**, use `Basic` authentication with your [GitHub Personal Access Token (PAT)](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens). Encode the token using `x-oauth-basic` as the username:
...
Environment compute (CPU, memory, sandbox execution) is **not billed** during the preview period. See [Pricing](/gemini-api/docs/pricing#pricing-for-agents) for agent token costs.
